...
def core(words):
	...
	return("No")
def perform():
	rawtext = text1.get("1.0","end")
	text1.delete("1.0","end")
	fullLis = rawtext.split("\n")
	for x in range(1,11):
		text1.insert("end", "{0}. {1}\n".format(x, core(fullLis[(x-1)*5])))
...
root = tk.Tk()
root["bg"] = "light green"
root.title("'Better Than Kate' Bot V5")
root.resizable(False, False)
text1 = tk.Text(root)
text1.grid(row=0, column=0, columnspan=4, rowspan=5, padx=5, pady=5)
buttonSub = ttk.Button(root, text="Submit", command=perform)
buttonSub.grid(row=5, column=1)
buttonR = ttk.Button(root, text="Reset", command=reset)
